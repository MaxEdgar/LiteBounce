...
object HttpClient {
    ...
    @JvmField
    val DEFAULT_AGENT = "${Coffee.CLIENT_NAME}/${Coffee.clientVersion}" +
        " (${Coffee.clientCommit}, ${Coffee.clientBranch}, " +
        "${if (Coffee.IN_DEVELOPMENT) "dev" else "release"}, ${System.getProperty("os.name")})"
        ...
        .followSslRedirects(true).apply {
            try {
                val file = File(
                    ...
                    "${Coffee.CLIENT_NAME.lowercase(Locale.ROOT)}_http_cache",
                )
                file.mkdirs()
