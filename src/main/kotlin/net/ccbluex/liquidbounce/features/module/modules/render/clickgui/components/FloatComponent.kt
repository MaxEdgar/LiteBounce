...
/**
 * Slider component for float settings.
 * Adapted from Wurst7's SliderComponent.
 */
class FloatComponent(private val setting: RangedValue<Float>) : Component() {
    ...
    override fun handleMouseClick(mouseX: Double, mouseY: Double, mouseButton: Int, context: MouseButtonEvent) {
        val hasControlDown = context.hasControlDown()
        if (mouseY < getY() + 11) return

        when (mouseButton) {
            GLFW.GLFW_MOUSE_BUTTON_LEFT -> {
                if (hasControlDown) {
                    // Could open a text input screen here
                } else {
                    dragging = true
                }
            }
    ...
    private fun handleDragging(mouseX: Int, x3: Int, x4: Int) {
        ...
        val sliderStartX = x3.toDouble()
        val sliderWidth = (x4 - x3).toDouble()
        val mousePercentage = (mouseX - sliderStartX) / sliderWidth

        val min = (setting.range as ClosedFloatingPointRange<Float>).start
        val max = (setting.range as ClosedFloatingPointRange<Float>).endInclusive
        val range = max - min
        val value = min + range * mousePercentage.toFloat()
    ...
    override fun render(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val max = (setting.range as ClosedFloatingPointRange<Float>).endInclusive
        val range = max - min
        val percentage = if (range == 0f) 0f else (setting.get() - min) / range
        ...
        val valueStr = String.format("%.2f%s", setting.get(), setting.suffix)
    ...
    override fun getDefaultWidth(): Int {
        val nameWidth = font.width(setting.name)
        val valueWidth = font.width(String.format("%.2f%s", setting.get(), setting.suffix))
