...
/**
 * Component for keybind settings.
 * Adapted from Wurst7's concept.
 */
class BindComponent(private val bindSetting: Value<*>) : Component() {
    ...
    override fun render(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        if (hovering) gui.tooltip = bindSetting.description.get() ?: ""
        ...
        val name = bindSetting.name
        val valueStr = if (listening) "..." else bindSetting.get().toString()
    ...
    override fun getDefaultWidth(): Int {
        val nameWidth = font.width(bindSetting.name)
        val valueWidth = font.width(bindSetting.get().toString())
        return nameWidth + valueWidth + 10
    }
