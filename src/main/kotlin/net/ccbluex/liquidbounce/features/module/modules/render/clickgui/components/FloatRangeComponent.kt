...
/**
 * Component for float range pair settings (min..max).
 * Shows the values as text.
 */
class FloatRangeComponent(private val setting: RangedValue<ClosedFloatingPointRange<Float>>) : Component() {
    ...
    override fun render(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val valueStr = String.format("%.2f..%.2f%s", range.start, range.endInclusive, setting.suffix)
